...
import {
  Users, TrendingUp, Settings, Download,
  Eye, FileText, Filter, QrCode, MapPin,
  Clock, Search, X, BarChart2,
} from "lucide-react";
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; cls: string; dot: string }> = {
    active:   { label: "Active",   cls: "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-500/15 dark:text-emerald-400 dark:border-emerald-500/30", dot: "bg-emerald-500 animate-pulse" },
    expired:  { label: "Expired",  cls: "bg-red-50 text-red-700 border-red-200 dark:bg-red-500/15 dark:text-red-400 dark:border-red-500/30", dot: "bg-red-500" },
    archived: { label: "Archived", cls: "bg-slate-100 text-slate-600 border-slate-200 dark:bg-slate-500/15 dark:text-slate-400 dark:border-slate-500/30", dot: "bg-slate-400" },
  };
  ...
  return (
    ...
    >
      ...
      <div className={`absolute left-0 top-0 bottom-0 w-[3px] rounded-l-2xl ${
        ...
        effectiveStatus === "expired" ? "bg-red-500" : "bg-slate-300 dark:bg-slate-600"
      }`} />
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
          ...
          <div className="flex items-start gap-3 flex-1 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-[10px] font-mono text-cyan-600 dark:text-cyan-400/80 mb-0.5 tracking-wider">EMP-{passNumber}</p>
              <h3 className="font-bold text-slate-900 dark:text-white text-sm leading-tight truncate">{licensee}</h3>
              <p className="text-xs text-slate-500 mt-0.5">{mineral} • {quantity} T</p>
            </div>
          </div>

          {/* Meta row */}
          <div className="flex items-center gap-3 flex-wrap sm:contents">
            <div className="flex items-center gap-1.5 sm:min-w-[120px]">
              <MapPin className="w-3 h-3 text-slate-400 shrink-0" />
              <span className="text-xs text-slate-600 dark:text-slate-300 truncate max-w-[100px] sm:max-w-none">{destination}</span>
            </div>
            <StatusBadge status={effectiveStatus} />
            <div className="flex items-center gap-1.5 text-xs text-slate-500 sm:min-w-[130px]">
              <Clock className="w-3 h-3 shrink-0" />
              <span>{validUptoDate ? validUptoDate.toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" }) : "—"}</span>
            </div>
            <div className="text-xs text-slate-400 hidden sm:block">
              Scans: <span className="font-semibold text-slate-600 dark:text-slate-300">{record.total_scans || 0}</span>
            </div>
          </div>

          {/* Actions */}
          <div className="flex items-center gap-1.5 border-t border-slate-100 dark:border-white/[0.04] pt-3 sm:border-0 sm:pt-0">
            <Link href={`/records/${record.id}`} className="p-2 rounded-lg text-slate-400 hover:text-cyan-500 dark:hover:text-cyan-400 hover:bg-cyan-500/10 transition-all" title="View">
...
function SkeletonRow() {
  return (
    <div className="rounded-2xl border border-slate-200 dark:border-white/[0.05] bg-slate-50 dark:bg-white/[0.02] p-4 animate-pulse">
...
export default function HostDashboardUI() {
  ...
  const filteredRecords = records.filter((record) => {
    ...
    if (searchQuery.trim()) {
      ...
      const licensee  = getFormValue(record.form_data, ["name_of_licensee", "licensee_name", "nameOfLicenseeOfLease"]).toLowerCase();
      ...
      if (!licensee.includes(q) && !mineral.includes(q) && !passNo.includes(q)) return false;
    }
    return true;
  });
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white pt-16 transition-colors duration-300">
      ...
      <div className="border-b border-slate-200 dark:border-white/[0.05]
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-3">
          <div>
            <h1 className="text-lg sm:text-xl font-black text-slate-900 dark:text-white">License Portal</h1>
            <p className="text-xs text-slate-500 mt-0.5 hidden sm:block">All mineral transportation passes</p>
          </div>
          <div className="flex items-center gap-2">
            <button
              ...
              className="hidden sm:flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold
                         ...
                         hover:border-slate-300 dark:hover:border-white/[0.15] transition-all"
            >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-5 sm:py-8 space-y-5 sm:space-y-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          <StatCard label="Total Passes"  value={records.length} icon={<FileText className="w-4 h-4 sm:w-5 sm:h-5" />}   accent="text-cyan-500 dark:text-cyan-400"    bg="bg-cyan-500/10"    border="border-cyan-500/20"    delay={0} />
          <StatCard label="Active"        value={activeCount}    icon={<TrendingUp className="w-4 h-4 sm:w-5 sm:h-5" />}  accent="text-emerald-600 dark:text-emerald-400" bg="bg-emerald-500/10" border="border-emerald-500/20" delay={0.05} />
          <StatCard label="Total Users"   value={totalUsers}     icon={<Users className="w-4 h-4 sm:w-5 sm:h-5" />}       accent="text-violet-600 dark:text-violet-400"  bg="bg-violet-500/10"  border="border-violet-500/20"  delay={0.1} />
          <StatCard label="This Month"    value={thisMonthCount} icon={<QrCode className="w-4 h-4 sm:w-5 sm:h-5" />}      accent="text-blue-600 dark:text-blue-400"      bg="bg-blue-500/10"    border="border-blue-500/20"    delay={0.15} />
        </div>
          ...
          {showCharts && (
            ...
            >
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div className="lg:col-span-2 rounded-2xl border border-slate-200 dark:border-white/[0.06] bg-slate-900 p-5 sm:p-6 shadow-sm">
                  <LineChart
                    data={monthlyData}
                    title="Pass Activity"
                ...
                <div className="rounded-2xl border border-slate-200 dark:border-white/[0.06] bg-slate-900 p-5 sm:p-6 flex flex-col items-center justify-center shadow-sm min-h-[220px]">
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-4">Status Breakdown</p>
                  <DonutChart slices={donutSlices} centerLabel={String(records.length)} centerSub="Total" />
                </div>
              </div>
            </motion.div>
          )}
        ...
        <div className="rounded-2xl border border-slate-200 dark:border-white/[0.06]
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-white/[0.06] text-sm text-slate-800 dark:text-slate-200 placeholder:text-slate-400 focus:outline-none focus:border-cyan-400 dark:focus:border-cyan-500/40 transition-colors"
              />
            </div>
          ...
          <div className="flex flex-wrap items-end gap-2 sm:gap-3">
            {["From", "To"].map((label, li) => {
              ...
              const setter = li === 0
                ? (v: string) => { setDateFrom(v); if (dateTo && v && new Date(v) > new Date(dateTo)) setDateTo(v); }
                : (v: string) => { setDateTo(v);   if (dateFrom && v && new Date(v) < new Date(dateFrom)) setDateFrom(v); };
              return (
                <div key={label}>
            ...
            <div className="flex gap-2 flex-wrap">
              {[
                ...
                <button key={p.label} onClick={p.action}
                  className="px-2.5 sm:px-3 py-2 rounded-xl text-xs font-semibold bg-slate-100 dark:bg-slate-800/60 border border-slate-200 dark:border-white/[0.05] text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:border-slate-300 dark:hover:border-white/[0.12] transition-all"
                >
                  {p.label}
                </button>
              ))}
            ...
            <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-100 dark:border-white/[0.04]">
              ...
              {filterStatus !== "All" && (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-cyan-50 dark:bg-cyan-500/10 border border-cyan-200 dark:border-cyan-500/20 text-[11px] font-medium text-cyan-700 dark:text-cyan-400">
                  Status: {filterStatus}
                  <button onClick={() => setFilterStatus("All")}><X className="w-3 h-3 ml-1" /></button>
                </span>
              )}
              {searchQuery && (
              ...
              {(dateFrom || dateTo) && (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-violet-50 dark:bg-violet-500/10 border border-violet-200 dark:border-violet-500/20 text-[11px] font-medium text-violet-700 dark:text-violet-400">
                  {dateFrom || "…"} → {dateTo || "…"}
                  <button onClick={() => { setDateFrom(""); setDateTo(""); }}><X className="w-3 h-3 ml-1" /></button>
                </span>
              )}
        ...
        {loading ? (
          ...
          <motion.div
            ...
            className="flex flex-col items-center justify-center py-20 rounded-2xl
                       ...
                       bg-slate-50 dark:bg-white/[0.02] text-center"
          >
            ...
            <h3 className="text-base font-bold text-slate-800 dark:text-white mb-2">No passes found</h3>
            <p className="text-sm text-slate-500 max-w-xs">
              {filterStatus === "All" ? "No mineral transport passes exist yet." : `No passes match the "${filterStatus}" filter.`}
            </p>
            {filterStatus !== "All" && (
              <button onClick={() => setFilterStatus("All")} className="mt-4 text-sm text-cyan-600 dark:text-cyan-400 hover:text-cyan-700 dark:hover:text-cyan-300 font-semibold">
                Clear filter
              </button>
            )}
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2.5 sm:space-y-3">
            ...
            <div className="hidden sm:grid grid-cols-[2fr_1.5fr_1fr_1.5fr_auto] gap-4 px-5 py-1.5">
              {["Pass / Licensee", "Destination", "Status", "Valid Until", "Actions"].map((h) => (
            ...
            {filteredRecords.map((record, i) => {
              ...
              return (
                <RecordRow
                  ...
                  licensee={getFormValue(fd, ["name_of_licensee", "licensee_name", "nameOfLicenseeOfLease"], "Unknown Licensee")}
                  mineral={getFormValue(fd, ["name_of_mineral", "mineral_name", "mineralName"], "N/A")}
