  ...
  return (
    ...
    >
      ...
      {filterStatus !== "All" ? (
        ...
        <Link
          href="/form"
          className="inline-flex items-center gap-2 px-5 py-2.5 bg-linear-to-r from-cyan-500 to-blue-600 rounded-xl font-semibold text-white text-sm hover:shadow-lg hover:shadow-cyan-500/20 transition-all"
        >
...
function SkeletonCard() {
  return (
    <div className="rounded-2xl border border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-white/2 p-4 sm:p-5 animate-pulse">
...
export default function UserDashboard() {
  ...
  const filteredRecords = records.filter((record) => {
    ...
    if (searchQuery.trim()) {
      ...
      const dest = getFormValue(record.form_data, [
        "destination_delivery_address",
        "destination_district",
        "nameOfConsignee",
      ]).toLowerCase();
      if (!mineral.includes(q) && !passNo.includes(q) && !dest.includes(q))
        return false;
    }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5 hidden sm:block">
              Mineral transportation dashboard
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Link
              href="/form"
              className="flex items-center gap-2 px-3 sm:px-4 py-2 rounded-xl bg-linear-to-r from-cyan-500 to-blue-600 text-white font-semibold text-sm hover:shadow-lg hover:shadow-cyan-500/25 transition-all"
            >
              <Plus className="w-4 h-4" />
          ...
          {showCharts && (
            ...
            >
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                {/* Line chart */}
                <div
                  className="lg:col-span-2 rounded-2xl border border-slate-200 dark:border-white/6
                ...
                <div
                  className="rounded-2xl border border-slate-200 dark:border-white/6
                                bg-slate-900 dark:bg-slate-900
                                p-5 sm:p-6 flex flex-col items-center justify-center shadow-sm min-h-55"
                >
              ...
              {filterStatus !== "All" && (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-cyan-50 dark:bg-cyan-500/10 border border-cyan-200 dark:border-cyan-500/20 text-[11px] font-medium text-cyan-700 dark:text-cyan-400">
                  ...
                  <button
                    ...
                    className="ml-1 hover:text-cyan-900 dark:hover:text-white"
                  >
                    <X className="w-3 h-3" />
                  </button>
                </span>
              ...
              {(dateFrom || dateTo) && (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-violet-50 dark:bg-violet-500/10 border border-violet-200 dark:border-violet-500/20 text-[11px] font-medium text-violet-700 dark:text-violet-400">
                  ...
                  <button
                    ...
                    className="ml-1 hover:text-violet-900 dark:hover:text-white"
                  >
                    <X className="w-3 h-3" />
                  </button>
                </span>
