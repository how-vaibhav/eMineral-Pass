...
function Toast({ type, message, onClose }: { type: "success" | "error"; message: string; onClose: () => void }) {
  return (
    <motion.div
      ...
      className={`fixed top-20 right-4 z-50 flex items-center gap-3 px-4 py-3 rounded-xl border shadow-2xl backdrop-blur-sm ${
        type === "success"
...
function EmptyState({ filterStatus, onClearFilter }: { filterStatus: FilterStatus; onClearFilter: () => void }) {
  return (
    <motion.div
      ...
      className="flex flex-col items-center justify-center py-24 px-6 text-center rounded-2xl border border-white/[0.05] bg-white/[0.02]"
    >
      ...
      <h3 className="text-lg font-bold text-white mb-2">No passes found</h3>
      ...
      {filterStatus !== "All" ? (
        <button onClick={onClearFilter} className="text-sm text-cyan-400 hover:text-cyan-300 font-semibold transition-colors">
          Clear filter
        </button>
      ) : (
        <Link
...
export default function UserDashboard() {
  ...
  const donutSlices = [
    ...
    { label: "Archived", value: records.length - activeCount - expiredCount, color: "#475569" },
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white pt-16">
      ...
      {/* ── Page header ─────────────────────────────────────────────────────── */}
      <div className="border-b border-white/[0.05] bg-slate-900/60 backdrop-blur-md sticky top-16 z-30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between gap-4">
          <div>
            <h1 className="text-xl font-black text-white">My Passes</h1>
            <p className="text-xs text-slate-500 mt-0.5">Mineral transportation dashboard</p>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setShowCharts((v) => !v)}
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold border border-white/[0.07] bg-white/[0.03] text-slate-400 hover:text-white hover:border-white/[0.15] transition-all"
            >
            ...
            <Link
              ...
              className="p-2 rounded-xl border border-white/[0.07] bg-white/[0.03] text-slate-400 hover:text-white hover:border-white/[0.15] transition-all"
              title="Settings"
            >
            ...
            <Link
              href="/form"
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-semibold text-sm hover:shadow-lg hover:shadow-cyan-500/25 transition-all"
            >
              <Plus className="w-4 h-4" />
              <span>New Pass</span>
            </Link>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8">
          ...
          {showCharts && (
            ...
            >
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                {/* Donut chart */}
                <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] backdrop-blur-sm p-6 flex flex-col items-center justify-center">
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-4">Status Breakdown</p>
                  <DonutChart
                    slices={donutSlices}
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] backdrop-blur-sm p-4 space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="relative flex-1 min-w-[180px]">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 rounded-xl bg-slate-800/60 border border-white/[0.06] text-sm text-slate-200 placeholder:text-slate-600 focus:outline-none focus:border-cyan-500/40 transition-colors"
              />
            </div>
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition-all ${
                    filterStatus === s
                      ...
                      : "bg-slate-800/60 border border-white/[0.05] text-slate-500 hover:text-white hover:border-white/[0.12]"
                  }`}
                >
          ...
          <div className="flex flex-wrap items-end gap-3">
            <div>
              <label className="block text-[10px] font-semibold text-slate-600 uppercase tracking-wider mb-1">From</label>
              <input
                type="date"
                value={dateFrom}
                onChange={(e) => {
                ...
                className="px-3 py-2 rounded-xl bg-slate-800/60 border border-white/[0.06] text-sm text-slate-200 focus:outline-none focus:border-cyan-500/40 transition-colors"
              />
            </div>
            <div>
              <label className="block text-[10px] font-semibold text-slate-600 uppercase tracking-wider mb-1">To</label>
              <input
                type="date"
                value={dateTo}
                onChange={(e) => {
            ...
            <div className="flex gap-2 flex-wrap">
              {[
                ...
                <button
                  key={p.label}
                  onClick={p.action}
                  className="px-3 py-2 rounded-xl text-xs font-semibold bg-slate-800/60 border border-white/[0.05] text-slate-400 hover:text-white hover:border-white/[0.12] transition-all"
                >
                  {p.label}
                </button>
              ))}
              {(dateFrom || dateTo) && (
                <button
                  ...
                  className="px-3 py-2 rounded-xl text-xs font-semibold text-slate-500 hover:text-white transition-colors"
                >
                  Clear
                </button>
              )}
          ...
          {(filterStatus !== "All" || dateFrom || dateTo || searchQuery) && (
            <div className="flex flex-wrap gap-2 pt-1 border-t border-white/[0.04]">
              <span className="text-[10px] font-semibold text-slate-600 uppercase tracking-widest self-center">Active filters:</span>
              {filterStatus !== "All" && (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-cyan-500/10 border border-cyan-500/20 text-[11px] font-medium text-cyan-400">
                  Status: {filterStatus}
                  <button onClick={() => setFilterStatus("All")} className="ml-1 hover:text-white"><X className="w-3 h-3" /></button>
                </span>
              )}
              {searchQuery && (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-blue-500/10 border border-blue-500/20 text-[11px] font-medium text-blue-400">
                  Search: {searchQuery}
                  <button onClick={() => setSearchQuery("")} className="ml-1 hover:text-white"><X className="w-3 h-3" /></button>
                </span>
              )}
              {(dateFrom || dateTo) && (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-violet-500/10 border border-violet-500/20 text-[11px] font-medium text-violet-400">
                  {dateFrom || "…"} → {dateTo || "…"}
                  <button onClick={() => { setDateFrom(""); setDateTo(""); }} className="ml-1 hover:text-white"><X className="w-3 h-3" /></button>
                </span>
              )}
              <span className="text-[11px] text-slate-600 self-center ml-auto">
                {filteredRecords.length} of {records.length} passes
              </span>
            </div>
          )}
        </div>
        ...
        {loading ? (
          ...
          >
            {/* Table header (md+) */}
            <div className="hidden sm:grid grid-cols-[2fr_1.5fr_1fr_1.5fr_auto] gap-4 px-5 py-2">
