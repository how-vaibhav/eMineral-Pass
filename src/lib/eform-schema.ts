...
export const EFORM_C_SCHEMA: FormSchema = {
  ...
  fields: [
...

    {
      ...
      type: "text",
      required: true,
      placeholder: "e.g., 50 MT, 100 Tonnes, 25.5 Units",
...

    {
      ...
      type: "text",
      required: true,
      placeholder: "e.g., 50 km, 75.5 KM, 100 Kilometers",
      validation: { minLength: 1, maxLength: 100 },
    },
...
export function getEditableFormFields(): FormFieldDefinition[] {
  return EFORM_C_SCHEMA.fields.filter((field) => field.name !== "generated_on");
}
  ...
  const sections: FormSection[] = [
    {
      ...
      fields: EFORM_C_SCHEMA.fields.filter((f) =>
        ["eform_c_number"].includes(f.name),
      ),
    },
    {
